...
class ResolveEnvironment:
    ...
    def detect_resolve_installation(self) -> Optional[str]:
        """
    ...
    def _get_search_paths(self) -> List[Path]:
        ...
        if self.system == "windows":
            return [
                ...
                Path(os.environ.get("PROGRAMFILES(X86)", "C:/Program Files (x86)")) / "Blackmagic Design" / "DaVinci Resolve",
            ]
    ...
    def _find_resolve_executable(self, base_path: Path) -> Optional[Path]:
    ...
    def validate_resolve_version(self, resolve_path: str) -> Optional[str]:
        """
    ...
    def _get_windows_version(self, resolve_path: str) -> Optional[str]:
    ...
    def _get_macos_version(self, resolve_path: str) -> Optional[str]:
    ...
    def _get_linux_version(self, resolve_path: str) -> Optional[str]:
        """Get version from Linux executable."""
        try:
            # Try running with --version flag
            result = subprocess.run(
                [resolve_path, "--version"],
                capture_output=True,
                text=True,
                timeout=10
            )
    ...
    def check_resolve_running(self) -> bool:
        ...
        """
        resolve_processes = [
            "Resolve.exe", "DaVinciResolve.exe",  # Windows
            "Resolve", "DaVinci Resolve",         # macOS/Linux
        ]
    ...
    def _get_api_paths(self) -> Tuple[Optional[Path], Optional[Path]]:
