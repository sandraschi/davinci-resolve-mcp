...
class ColorNode(ResolveObject):
    ...
    def add_parameter(self, name: str, value: Any, param_type: str = "float") -> None:
        ...
        """
        self.parameters[name] = {"value": value, "type": param_type, "default": value}
...
class ColorGrade(ResolveObject):
    ...
    is_shared: bool = Field(default=False, description="Shared status")
    is_read_only: bool = Field(default=False, description="Read-only status")

    def add_node(self, node: ColorNode, parent_id: str | None = None) -> str:
        """
    ...
    def get_node(self, node_id: str) -> ColorNode | None:
        """
        Get a node by ID.
...
class LUTInfo(ResolveObject):
    ...
    color_space: ColorSpace = Field(default=ColorSpace.REC709, description="Target color space")
    input_color_space: ColorSpace | None = Field(default=None, description="Input color space")
    output_color_space: ColorSpace | None = Field(default=None, description="Output color space")
    is_builtin: bool = Field(default=False, description="Built-in LUT status")
...
class ColorWheels(ResolveObject):
    ...
    lift: tuple[float, float, float, float] = Field(
        default=(0.0, 0.0, 0.0, 0.0), description="Lift (shadows) RGBA values"
    )
    gamma: tuple[float, float, float, float] = Field(
        default=(0.0, 0.0, 0.0, 0.0), description="Gamma (midtones) RGBA values"
    )
...
class ScopesSettings(ResolveObject):
    ...
    rgb_overlay: bool = Field(default=False, description="Show RGB overlay")
    y_only: bool = Field(default=False, description="Show Y-only")
    show_skin_tone_indicator: bool = Field(default=True, description="Show skin tone indicator")
    show_clipping: bool = Field(default=True, description="Show clipping indicators")
    show_legal_lines: bool = Field(default=True, description="Show legal lines")
    show_legal_matte: bool = Field(default=False, description="Show legal matte")
    show_cursor_position: bool = Field(default=True, description="Show cursor position")
    show_audio_waveform: bool = Field(default=False, description="Show audio waveform")
