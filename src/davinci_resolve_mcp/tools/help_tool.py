...
class HelpTool:
    ...
    def _add_task_help(
        ...
        help_content = HelpContent(
            name=name, description=description, content=content, examples=examples, related=related
        )
...
"""
        ...
        categories = {
            ...
            "Advanced": ["color_grading", "audio_mixing", "render_settings"],
        }
...
async def resolve_help(topic: Optional[str] = None, level: str = "intermediate") -> str:
    ...
    This provides multi-level, context-aware help on how to use the server.
    It documents the various core classes (Project, Timeline, etc) and common tasks.

    Args:
        topic: The topic to get help for (e.g., "create_project", "Timeline"). If omitted, returns an overview.
        level: The desired depth of the manual: "beginner", "intermediate", "advanced", or "developer".
    """
    # Use the programmatic helper to format the output
    return get_help(topic=topic, level=level)
