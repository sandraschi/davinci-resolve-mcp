import { Loader2, Mic, MicOff, Music2, Volume2, VolumeX } from "lucide-react";
...
export function Fairlight() {
  ...
  const openPage = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/fairlight/open-page`, {
        method: "POST",
      });
      if (!res.ok) throw new Error(await res.text());
      await res.json();
  ...
  const setMute = async (trackIndex: number, mute: boolean) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/fairlight/track/mute?track_index=${trackIndex}&mute=${mute}`, {
        method: "POST",
      });
      if (!res.ok) throw new Error(await res.text());
      await fetchTracks();
  ...
  const setSolo = async (trackIndex: number, solo: boolean) => {
    ...
    setActionLoading((prev) => ({ ...prev, [key]: true }));
    try {
      const res = await fetch(`${API_BASE}/fairlight/track/solo?track_index=${trackIndex}&solo=${solo}`, {
        method: "POST",
      });
      if (!res.ok) throw new Error(await res.text());
      await fetchTracks();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          >
            {actionLoading.openPage ? <Loader2 className="h-4 w-4 animate-spin" /> : "Open Fairlight page in Resolve"}
          </Button>
          <Button
            variant="outline"
            size="sm"
      ...
      {error && (
        <div className="rounded-lg border border-red-900/50 bg-red-950/20 p-4 text-red-300 text-sm">{error}</div>
      )}
        ...
        <Card className="border-slate-800 bg-slate-900/50">
          <CardContent className="pt-6">
            <p className="text-slate-400">
              Not connected to DaVinci Resolve. Start Resolve and ensure the MCP backend is connected.
            </p>
          </CardContent>
        </Card>
      )}
            ...
            <div className="space-y-2">
              {data.tracks.map((track) => (
                ...
                >
                  ...
                  {track.locked && <span className="text-xs text-slate-500">Locked</span>}
                </div>
              ))}
            </div>
          </CardContent>
